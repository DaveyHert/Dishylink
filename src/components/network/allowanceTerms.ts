...
//
// Split from the fields that render them because both the device card and the
// group it can become need these, and a file mixing components with the values
// they share cannot be hot-reloaded.
...
/** The countdown as its two fields read it. */
...
/**
 * Devices in the order a picker holds them still.
 *
 * Active first, as asked, but sorted by name within each band rather than by
 * anything that moves. Ordering live devices by traffic or by last seen re-sorts
 * them on every poll, which leaves someone part-way down the list chasing the row
 * they were about to tick.
 */
...
/**
 * The editable terms of one allowance.
 *
 * `startedMs` seeds a custom cycle's first boundary, and `billingDay` is the
 * account's own reset day — without one, the option that needs it is refused
 * rather than silently measuring a calendar month instead.
 */
